...
if sim_options.FineTimeSync
   ...
   long_tr = round(sim_consts.ModScale *  sim_consts.LongTrainingSymbols);
   long_tr_symbols = tx_freqd_to_timed(long_tr,sim_options);
   
   %long_tr_symbols = tx_freqd_to_timed(sim_consts.LongTrainingSymbols,sim_options);
   ...
   time_corr_long = floor(sum(abs(time_corr_long),1)/2^10);
