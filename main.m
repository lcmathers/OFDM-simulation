...
Np=2; % the space of pilot symbol
...
M_bit=log2(M_mod);
bits_perframe=M_bit*Ndata*Nframe;
...
for i=1:length(SNR_dB)
    for iter=1:max_iter
        ...
        raw_reshape=reshape(raw_data,M_bit,Ndata*Nframe);

        X_data=A*qammod(raw_reshape,M_mod,'gray','InputType','bit');  

        x_tr=frame_generate(X_data,Nfft,Nsym,Ndata,Nvc,Nframe,Train_sym);  % generate a frame
        ...
        % 
        Y_est_data=frame_decompose(y_data,Nfft,Nsym,Ndata,Nvc,Nframe,channeltype,h);
