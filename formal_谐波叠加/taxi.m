...
function [mu_k,t_domain,fs1,delay_max]=taxi(T_sim,fd,K_taxi_1,f_los_taxi,fs,delay_actual,delay_actual_dB,N_i)
%% 参数设定

%T_sim=0.1;  % 信道响应仿真时间
%N_i=20;  % 最小的谐波数 每一径的谐波叠加的数目不同
%fd=3000; % 最大多普勒频率 
%K_taxi=6.9;   %滑行场景的莱斯因子 (dB)
%K_taxi_1=10^(K_taxi/10);

%f_los_taxi=0.7*fd;  % LOS径的频偏 0.7倍的最大频偏
...
%fs1=100*fd;
[mu_k,t_domain]=SOS_MEDS(F1,F2,C1,C2,Th1,Th2,K_taxi_1,f_los_taxi,T_sim,fs1);
